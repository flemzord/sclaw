...
1. **Reason** — The LLM receives the conversation history (system prompt + messages + tool results) and generates a response.
...
The agent loop supports streaming via `RunStream()`, which returns a channel of `StreamEvent` values:
...
<Tip>
Streaming is used by channel adapters to provide real-time response updates. For example, the Telegram channel edits the message progressively as chunks arrive.
</Tip>
