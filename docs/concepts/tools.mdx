...
In addition to tool definitions, the pipeline injects the agent's **workspace directory path** into the system prompt (Step 9c). This tells the LLM where file operations and command execution happen, so it can confidently resolve both relative and absolute paths.
