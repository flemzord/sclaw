...
func (t *readFileTool) Execute(_ context.Context, args json.RawMessage, env tool.ExecutionEnv) (tool.Output, error) {
	...
	resolved, err := safepath.ForRead(env.Workspace, env.DataDir, a.Path, env.PathFilter)
	if err != nil {
	...
	if info.Size() > int64(t.maxFileSize) {
		return tool.Output{Content: fmt.Sprintf("file too large: %d bytes (max %d)", info.Size(), t.maxFileSize), IsError: true}, nil
	}
