...
// Module implements a configurable file write tool module.
type Module struct {
	config Config
	tool   *writeFileTool
}
...
func (m *Module) Configure(node *yaml.Node) error {
	if err := node.Decode(&m.config); err != nil {
		return fmt.Errorf("file_write: decode config: %w", err)
	}
	return nil
}
...
func (m *Module) Provision(_ *core.AppContext) error {
	...
	m.tool = &writeFileTool{
		maxFileSize: m.config.MaxFileSize,
		createDirs:  m.config.createDirsEnabled(),
