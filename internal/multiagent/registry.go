...
// Resolution cascade: user -> thread -> group -> channel -> default -> error.
type Registry struct {
	agents       []agentEntry
	userIndex    map[string][]int // sender ID -> agent entry indices
	threadIndex  map[threadRouteKey][]int
...
func parseThreadRoute(route string) (threadRouteKey, bool) {
	chatID, threadID, ok := strings.Cut(route, ":")
	if !ok || chatID == "" || threadID == "" {
		return threadRouteKey{}, false
	}
	return threadRouteKey{
		ChatID:   chatID,
		ThreadID: threadID,
	}, true
}
...
func NewRegistry(agents map[string]AgentConfig, order []string) (*Registry, error) {
	r := &Registry{
		...
		threadIndex:  make(map[threadRouteKey][]int),
	...
	for i, id := range order {
		...
		for _, route := range cfg.Routing.Threads {
			if key, ok := parseThreadRoute(route); ok {
				r.threadIndex[key] = append(r.threadIndex[key], idx)
			}
		}
...
// It follows the cascade: user -> thread -> group -> channel -> default -> ErrNoMatchingAgent.
func (r *Registry) Resolve(msg message.InboundMessage) (string, error) {
	...
	// Priority 2: thread/topic filter
	if msg.ThreadID != "" {
		if indices, ok := r.threadIndex[threadRouteKey{ChatID: msg.Chat.ID, ThreadID: msg.ThreadID}]; ok && len(indices) > 0 {
			return r.agents[indices[0]].ID, nil
		}
	}
	// Priority 3: group filter
	if indices, ok := r.groupIndex[msg.Chat.ID]; ok && len(indices) > 0 {
		return r.agents[indices[0]].ID, nil
	}
	// Priority 4: channel filter
